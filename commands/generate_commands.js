...
async function generate_frontend_code(args) {
    try {
        ...
        let designPathPrompt = args.design ? `Use any image files ${args.design} as overall layout and styling references. Use any JSON files (e.g., 'header.json', 'main.json') as structured Figma exports to guide the generation of individual UI components.` : "";
        let outputpath = args.output||path.join(process.cwd(),"../output-"+v4());
        let outputPrompt = `write the files to given absolute folder path ${outputpath}`;
        let modifiedPrompt = `Generate a user registration form with email validation.${designPathPrompt}. ${outputPrompt}`
